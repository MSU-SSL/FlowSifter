...
let reduce_transitions defs qs center =
  ...
  in
  let rec defer p = MPM.find p defs |> PSet.iter (fun c -> if defer_to p c then defer c )
  in
...
let defer = Point.create "defer"
let gni_p = Point.create "get_next_id"
let gni_t = Time.create "get_next_id"
