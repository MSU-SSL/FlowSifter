...
type ('a,'b) vmstride_dstate = ('a, vs_map, 'b) defer_state

type ('a,'b) vmd2fa = ('a,'b) vmstride_dstate fa

type ('a,'b) t = ('a,'b) vmd2fa

let count map = Fdd.enum map |> Enum.count
let pf = Fdd.print (Pair.print2 Int.print)
let pfs str fdd = IO.nwrite stdout str; pf stdout fdd; IO.nwrite stdout "\n"

let vm_state_of_dstate dq =
  let vsmap = IMap.map Fdd.of_term dq.map |> Fdd.of_fdd_imap |> Fdd.map_dec (fun q -> q,1) in
  {dq with map = {var = vsmap; max=vsmap; stride = 1; one = dq.map}}

let of_d2fa : (('a,'b) d2fa -> ('a,'b) t) = fun d2fa ->
  map_qs vm_state_of_dstate d2fa
...
let incr_stride fdd = Fdd.map_dec (fun (q,s) -> q,(s+1)) fdd

let increase_stride1 is_empty_dec vmd q = 
  ...
  let fdd_branch qi = 	
    ...
    else Fdd.of_term (qi,1)
  in
  let fdd_branch,_ = cache fdd_branch 10 in
  let new_max = IMap.map fdd_branch q.map.one |> Fdd.of_fdd_imap in
  let new_var = Fdd.union ~min_key:0 ~max_key:(num_chars-1) (fun _a b -> b) q.map.var new_max in
(*  printf "State: %d\n" q.id;
...
let increase_stride1 is_empty_dec ~com_lim vmd q = 
  if complexity q > com_lim then q else increase_stride1 is_empty_dec vmd q 

let increase_stride1_all is_empty_dec ~com_lim vmd = 
  {vmd with qs = Array.map (increase_stride1 is_empty_dec ~com_lim vmd) vmd.qs}

let increase_stride_all is_empty_dec ~com_lim k vmd =
  power k (increase_stride1_all is_empty_dec ~com_lim) vmd

let boosted = ref 0

let boost_if_loop is_empty_dec ~loop_lim ~boost q =
  let boost_q q = 
    let new_stride = q.map.stride + 1 in
    let ext_one = incr_stride q.map.max in
    let fdd_branch qi = if qi <> q.id then Fdd.of_term (qi,1) else ext_one in
    let new_max = IMap.map fdd_branch q.map.one |> Fdd.of_fdd_imap in
    let new_var = Fdd.union ~min_key:0 ~max_key:(num_chars-1) (fun _a b -> b) q.map.var new_max in
    {q with map = {q.map with var = new_var; max = new_max; stride = new_stride}}
  in
...
let boost is_empty_dec ?(loop_lim=100) ~boost vmd =
  {vmd with qs = Array.map (boost_if_loop is_empty_dec ~loop_lim ~boost) vmd.qs}
...
let boosted_table_of_root boost q =
  ...
  Vect.concat table boost_rv
	   |> Vect.append default 
	   |> Fdd.make_rv ~default:(-1,0)

let boosted_table_of_q ?(boost=7) q = 
  if D2fa.is_root q then (
(*    printf "R%!";  *)
    boosted_table_of_root boost q 
      |> Optimizers.trazor ~bit_width:bits_per_char
  ) else (
(*    printf "D%!";  *)
    table_of_q q
      |> tcam_of_rs 
...
let rs_table ?(boost=7) vsdfa = Array.map (boosted_table_of_q ~boost) vsdfa.qs
let rs_table_lazy ?(boost=7) vsdfa = Array.enum vsdfa.qs |> map (boosted_table_of_q ~boost)
...
let to_rs_dfa ?(boost=7) vsdfa = Regex_dfa.map_qs (boosted_table_of_q ~boost |- trim_fields) vsdfa
...
let tcam_size vsdfa = Array.enum vsdfa.qs |> Enum.map (boosted_table_of_q ~boost:1 |- Vect.length) |> Enum.reduce (+)
