...
(*  Point.observe sod_p; Time.start sod_t; *)
  ...
  let decs = D.decs decider |> PSet.enum |> Array.of_enum in
(*  Time.stop sod_t; *)
  (bdd, decs, of_prefix)
...
let struct_of_tcam ?perm tcam =
  ...
  let decs = Tcam.decs tcam |> PSet.enum |> Array.of_enum in
  (bdd, decs, of_prefix)
...
let bitmerge_group g = 
  ...
  let ret = ref (PSet.create compare_r) in
  let merge i j = 
    ret := PSet.add (Entry.merge g.(i) g.(j)) !ret; 
...
let bitmerge tcam = if Vect.length tcam < 2 then tcam else
  ...
  let rec loop etcam =
    ...
    and entries = enum_map (fun (x,_) -> PSet.enum x) group_results in
